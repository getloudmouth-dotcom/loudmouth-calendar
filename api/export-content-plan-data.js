// /api/export-content-plan-data.js
// Serves content plan payload to Puppeteer using a short-lived Redis token.
...
async function handler(req, res) {
  ...
  let redis;
  try { redis = getRedis(); } catch (e) { return res.status(503).json({ error: e.message }); }

  const payload = await redis.get(`cpexport:${token}`);
  if (!payload) return res.status(404).json({ error: "Token expired or invalid" });

  // Single-use: delete immediately
  await redis.del(`cpexport:${token}`);
