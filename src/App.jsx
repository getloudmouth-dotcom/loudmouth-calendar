...
const ROLE_TOOLS = {
  admin:    ["calendar_creator", "content_scheduling", "admin_portal", "content_plan_creator"],
...
// ── Standalone: public client view (no auth required) ──
function ContentPlanPublicView({ token }) {
...
// ── Standalone: Puppeteer export view ──
function ContentPlanExportView({ token }) {
  const [plan, setPlan] = useState(null);
  const [items, setItems] = useState([]);
...
useEffect(() => {
  ...
  if (cpPublicToken) return <ContentPlanPublicView token={cpPublicToken} />;
