    ...
    <main style={AUTH_SHELL}>
      <div style={{ ...AUTH_CARD, maxWidth: 360 }}>
        ...
        <div style={{ ...SEGMENT_GROUP, marginBottom: 24 }}>
          <button onClick={() => setAuthMode("login")} style={segmentBtn(authMode === "login")}>Log In</button>
          <button onClick={() => setAuthMode("signup")} style={segmentBtn(authMode === "signup")}>Sign Up</button>
        </div>

        <label htmlFor="auth-email" style={labelStyle}>Email</label>
        <input id="auth-email" type="email" placeholder="you@example.com" value={authEmail} onChange={e => setAuthEmail(e.target.value)} style={{ ...inputStyle, marginBottom: 16 }} />

        <label htmlFor="auth-password" style={labelStyle}>Password</label>
        <input id="auth-password" type="password" placeholder="••••••••" value={authPassword} onChange={e => setAuthPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} style={{ ...inputStyle, marginBottom: 16 }} />
