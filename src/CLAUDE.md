...
| Forms | `INPUT`, `LABEL` |
| Layout | `CARD`, `MODAL`, `BADGE`, `SECTION_HEADER`, `PAGE_HEADER`, `PAGE_TITLE`, `DISPLAY_TITLE`, `DISPLAY_SUBTITLE` |
...
- **For content-area section titles** (below the `PAGE_HEADER`, inside the page body), use `DISPLAY_TITLE` + `DISPLAY_SUBTITLE` inside a `div` with `marginBottom: 40`. Never write `fontSize: 28 / fontWeight: 700` inline.
