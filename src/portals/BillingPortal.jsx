...
import { SANS, MONO, C, INPUT, LABEL, ghostBtn, primaryBtn, dangerBtn, PAGE_HEADER, PAGE_TITLE, BTN_ROW } from "../theme";
...
function ClientCardSkeleton() {
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "20px 24px", display: "flex", flexDirection: "column", gap: 10 }}>
...
// ── Status badge ──────────────────────────────────────────────────────────────
const STATUS_COLORS = {
  draft:     { bg: C.surface2,  color: C.meta,    label: "Draft" },
  sent:      { bg: "#1e3a5f",   color: "#3B82F6", label: "Sent" },
  viewed:    { bg: "#2d1f5e",   color: "#8B5CF6", label: "Viewed" },
  paid:      { bg: "#1a2e0a",   color: C.accent,  label: "Paid" },
  overdue:   { bg: "#3a0a0a",   color: C.error,   label: "Overdue" },
  cancelled: { bg: C.surface,   color: C.meta,    label: "Cancelled" },
};
...
function FreshBooksStatusPill({ status, onReconnect }) {
  ...
  const color = ok ? C.accent : C.error;
...
export default function BillingPortal({ setActivePortal, deleteClient }) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.canvas, color: C.text, fontFamily: SANS }}>
      ...
      {apiError && (
        <div style={{ margin: "24px 48px 0", padding: "12px 16px", background: "#1a0000", border: "1px solid #330000", borderRadius: 8, color: C.error, fontSize: 13 }}>
          {apiError}
        </div>
      )}
      ...
      {tab === "invoices" && (
        <div style={{ padding: "32px 48px" }}>
          {invoiceSyncResult && (
            <div style={{ marginBottom: 16, padding: "10px 16px", background: invoiceSyncResult.error ? "#1a0000" : "#0d1a00", border: `1px solid ${invoiceSyncResult.error ? "#330000" : "#2a4e0a"}`, borderRadius: 8, color: invoiceSyncResult.error ? C.error : C.accent, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          {loadingI ? (
                ...
                <thead>
                  <tr style={{ borderBottom: `1px solid ${C.surface}` }}>
                    {["Invoice #", "Client", "Issue Date", "Due Date", "Total", "Status", "Actions"].map(h => (
                      <th key={h} style={{ textAlign: "left", color: C.meta, fontWeight: 600, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.06em", padding: "0 16px 12px 0" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
            ...
            <div style={{ color: C.meta, fontSize: 14, padding: "60px 0", textAlign: "center" }}>
                ...
                <tbody>
                  {invoices.map(inv => (
                    <tr key={inv.id} style={{ borderBottom: `1px solid ${C.border}` }}>
                      <td style={{ padding: "14px 16px 14px 0", color: C.text, fontWeight: 700, fontFamily: "monospace", fontSize: 13 }}>{inv.invoice_number}</td>
                      <td style={{ padding: "14px 16px 14px 0" }}>
                        <div style={{ color: C.text, fontWeight: 600 }}>{inv.clients?.name ?? "—"}</div>
                        {inv.clients?.company && <div style={{ color: C.meta, fontSize: 11 }}>{inv.clients.company}</div>}
                      </td>
                      <td style={{ padding: "14px 16px 14px 0", color: C.meta }}>{fmtDate(inv.issue_date)}</td>
                      <td style={{ padding: "14px 16px 14px 0", color: inv.status === "overdue" ? C.error : C.meta }}>{fmtDate(inv.due_date)}</td>
                      <td style={{ padding: "14px 16px 14px 0", color: C.accent, fontWeight: 700 }}>{fmt(inv.total, inv.currency)}</td>
                      ...
                      <td style={{ padding: "14px 0", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", gap: 6 }}>
                          ...
                          {["draft", "sent", "viewed", "overdue"].includes(inv.status) && (
                            <button onClick={() => { setSendModal(inv); setSendMethod("email"); setSendError(""); setSendSuccess(""); }} style={{ background: "transparent", color: C.meta, border: `1px solid ${C.border}`, borderRadius: 24, padding: "5px 12px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", letterSpacing: "1px", lineHeight: 1 }}>
                              Send
                            </button>
                          )}
                          {/* Mark Paid */}
                          {["sent", "viewed", "overdue"].includes(inv.status) && (
                            <button onClick={() => markPaid(inv)} style={{ background: "rgba(204,255,0,0.1)", color: C.accent, border: `1px solid ${C.accent}`, borderRadius: 24, padding: "5px 12px", fontSize: 10, fontWeight: 700, cursor: "pointer", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", letterSpacing: "1px", lineHeight: 1 }}>
                              Mark Paid
                            </button>
                          )}
                          {/* Payment link */}
                          {inv.payment_url && (
                            <a href={inv.payment_url} target="_blank" rel="noopener noreferrer" style={{ background: "transparent", color: C.meta, border: `1px solid ${C.border}`, borderRadius: 24, padding: "5px 12px", fontSize: 10, fontWeight: 700, cursor: "pointer", textDecoration: "none", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", letterSpacing: "1px", lineHeight: 1 }}>
                              FreshBooks ↗
                            </a>
                          )}
                        </div>
      ...
      {tab === "clients" && (
        <div style={{ padding: "32px 48px" }}>
          {syncResult && (
            <div style={{ marginBottom: 16, padding: "10px 16px", background: syncResult.error ? "#1a0000" : "#0d1a00", border: `1px solid ${syncResult.error ? "#330000" : "#2a4e0a"}`, borderRadius: 8, color: syncResult.error ? C.error : C.accent, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              {syncResult.error
          ...
          {loadingC ? (
            ...
            <div style={{ color: C.meta, fontSize: 14, padding: "60px 0", textAlign: "center", lineHeight: 1 }}>
              <div style={{ fontSize: 32, marginBottom: 12 }}>👤</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 16 }}>
              {clients.map(c => (
                <div key={c.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: "20px 24px" }}>
                  <div style={{ fontWeight: 800, fontSize: 16, color: C.text, marginBottom: 4, lineHeight: 1 }}>{c.name}</div>
                  {c.company && <div style={{ color: C.meta, fontSize: 12, marginBottom: 12, lineHeight: 1 }}>{c.company}</div>}
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {c.email && <div style={{ color: C.meta, fontSize: 12, display: "flex", gap: 8, lineHeight: 1 }}><span style={{ color: "rgba(255,255,255,0.4)" }}>Email</span>{c.email}</div>}
                    {c.phone && <div style={{ color: C.meta, fontSize: 12, display: "flex", gap: 8, lineHeight: 1 }}><span style={{ color: "rgba(255,255,255,0.4)" }}>Phone</span>{c.phone}</div>}
                  </div>
                  <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                        {c.freshbooks_contact_id ? (
                          <span style={{ fontSize: 9, color: C.accent, background: "rgba(204,255,0,0.12)", borderRadius: 20, padding: "2px 7px", fontWeight: 700, letterSpacing: "0.5px", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", lineHeight: 1 }}>FB Synced</span>
                        ) : (
                          <span style={{ fontSize: 9, color: C.meta, background: "rgba(255,255,255,0.07)", borderRadius: 20, padding: "2px 7px", fontWeight: 700, letterSpacing: "0.5px", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", lineHeight: 1 }}>Not Synced</span>
                        )}
                        {c.sms_consent_at ? (
                          <span style={{ fontSize: 9, color: C.success, background: "rgba(127,217,158,0.15)", borderRadius: 20, padding: "2px 7px", fontWeight: 700, letterSpacing: "0.5px", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", lineHeight: 1 }}>SMS Opted In</span>
                        ) : (
                          <span style={{ fontSize: 9, color: C.meta, background: "rgba(255,255,255,0.07)", borderRadius: 20, padding: "2px 7px", fontWeight: 700, letterSpacing: "0.5px", fontFamily: "'Space Mono', monospace", textTransform: "uppercase", lineHeight: 1 }}>SMS Not Opted In</span>
                        )}
                      </div>
                      <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
                        <button onClick={() => openEditClient(c)} style={{ background: "transparent", border: `1px solid ${C.border}`, borderRadius: 24, color: C.meta, fontSize: 10, padding: "4px 12px", cursor: "pointer", fontWeight: 700, fontFamily: "'Space Mono', monospace", textTransform: "uppercase", letterSpacing: "1px", lineHeight: 1 }}>
                          Edit
                        </button>
      ...
      {showClientForm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, width: "100%", maxWidth: 480, padding: 32 }}>
            <div style={{ fontWeight: 900, fontSize: 18, color: C.text, marginBottom: 24, lineHeight: 1 }}>{editingClient ? "Edit Client" : "New Client"}</div>
            <form onSubmit={submitClient} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {clientError && <div style={{ color: C.error, fontSize: 13, lineHeight: 1 }}>{clientError}</div>}
              <div style={{ ...BTN_ROW, marginTop: 8 }}>
                <button type="submit" disabled={savingClient} style={{ ...primaryBtn, opacity: savingClient ? 0.6 : 1, cursor: savingClient ? "default" : "pointer" }}>
                  {savingClient ? "Saving..." : editingClient ? "Save Changes" : "Create Client"}
                </button>
                <button type="button" onClick={() => setShowClientForm(false)} style={ghostBtn}>
                  Cancel
                </button>
              </div>
            </form>
      ...
      {showInvoiceForm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "40px 24px", overflowY: "auto" }}
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, width: "100%", maxWidth: 640, padding: 36, marginBottom: 40 }}>
            ...
            <form onSubmit={submitInvoice} style={{ display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {lineItems.map((li, i) => (
                    <div key={li._id} style={{ display: "grid", gridTemplateColumns: "1fr 72px 96px 32px", gap: 8, alignItems: "center" }}>
                      ...
                      <button type="button" onClick={() => removeLine(li._id)} style={{ background: "none", border: "none", color: C.meta, fontSize: 18, cursor: "pointer", padding: 0, textAlign: "center", lineHeight: 1 }} title="Remove">×</button>
                    </div>
                  ))}
                  <button type="button" onClick={() => setLineItems(prev => [...prev, emptyLine()])} style={{ background: "none", border: `1px dashed ${C.border}`, borderRadius: 8, color: C.meta, fontSize: 11, fontWeight: 700, padding: "8px 0", cursor: "pointer", marginTop: 4, fontFamily: "'Space Mono', monospace", textTransform: "uppercase", letterSpacing: "1px", lineHeight: 1 }}>
                    + Add Line Item
                  </button>
                </div>
                ...
                <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "120px 96px", gap: 8 }}>
                    <div style={{ fontSize: 11, color: C.meta, textAlign: "right", display: "flex", alignItems: "center", justifyContent: "flex-end", lineHeight: 1 }}>Tax %</div>
                  ...
                  <div style={{ fontSize: 12, color: C.meta, lineHeight: 1 }}>Subtotal: <strong style={{ color: C.text }}>{fmt(invoiceSubtotal())}</strong></div>
                  <div style={{ fontSize: 15, color: C.accent, fontWeight: 900, lineHeight: 1 }}>Total: {fmt(invoiceTotal())}</div>
                </div>
              </div>
              ...
              <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "16px 18px" }}>
                <label style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
                  <input type="checkbox" checked={invoiceForm.is_recurring} onChange={e => setInvoiceForm(f => ({ ...f, is_recurring: e.target.checked }))} style={{ width: 16, height: 16, accentColor: C.accent }} />
                  <span style={{ fontWeight: 700, fontSize: 13, color: C.text, lineHeight: 1 }}>Recurring Invoice</span>
                </label>
                {invoiceForm.is_recurring && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ fontSize: 11, color: C.meta, marginTop: 8, lineHeight: 1 }}>
              ...
              {invoiceError && <div style={{ color: C.error, fontSize: 13, lineHeight: 1 }}>{invoiceError}</div>}

              <div style={BTN_ROW}>
                <button type="submit" disabled={savingInvoice} style={{ ...primaryBtn, opacity: savingInvoice ? 0.6 : 1, cursor: savingInvoice ? "default" : "pointer" }}>
                  {savingInvoice ? "Creating..." : "Create Invoice"}
                </button>
                <button type="button" onClick={() => setShowInvoiceForm(false)} style={ghostBtn}>
                  Cancel
                </button>
              </div>
            </form>
      ...
      {sendModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, width: "100%", maxWidth: 440, padding: 32 }}>
            <div style={{ fontWeight: 900, fontSize: 18, color: C.text, marginBottom: 6, lineHeight: 1 }}>Send Invoice</div>
            <div style={{ color: C.meta, fontSize: 13, marginBottom: 24, lineHeight: 1 }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                {[
                  ...
                  return (
                    <button key={m.value} type="button" onClick={() => !disabled && setSendMethod(m.value)} title={disabled ? (noEmail ? "Client has no email" : "Client has no phone") : ""} style={{ flex: 1, background: active ? C.accent : C.surface2, color: active ? "#000" : disabled ? "rgba(255,255,255,0.2)" : C.meta, border: `1.5px solid ${active ? C.accent : C.border}`, borderRadius: 10, padding: "12px 0", fontWeight: 800, fontSize: 13, cursor: disabled ? "not-allowed" : "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, lineHeight: 1 }}>
            ...
            {/* Delivery preview */}
            <div style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 16px", fontSize: 12, color: C.meta, marginBottom: 20, lineHeight: 1.5 }}>
              {(sendMethod === "email" || sendMethod === "both") && sendModal.clients?.email && (
                <div style={{ lineHeight: 1 }}>Email → <span style={{ color: C.text }}>{sendModal.clients.email}</span></div>
              )}
              {(sendMethod === "sms" || sendMethod === "both") && sendModal.clients?.phone && (
                <div style={{ lineHeight: 1, marginTop: 6 }}>SMS → <span style={{ color: C.text }}>{sendModal.clients.phone}</span></div>
              )}
            </div>

            {sendError && <div style={{ color: C.error, fontSize: 13, marginBottom: 12, lineHeight: 1 }}>{sendError}</div>}
            {sendSuccess && <div style={{ color: C.accent, fontSize: 13, marginBottom: 12, fontWeight: 700, lineHeight: 1 }}>{sendSuccess}</div>}

            <div style={BTN_ROW}>
              <button onClick={sendInvoice} disabled={sending} style={{ ...primaryBtn, opacity: sending ? 0.6 : 1, cursor: sending ? "default" : "pointer" }}>
                {sending ? "Sending..." : `Send via ${sendMethod === "both" ? "Email & SMS" : sendMethod === "email" ? "Email" : "SMS"}`}
              </button>
              <button onClick={() => setSendModal(null)} disabled={sending} style={{ ...ghostBtn, opacity: sending ? 0.5 : 1 }}>
                Cancel
              </button>
            </div>
          </div>
